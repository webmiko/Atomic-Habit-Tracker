...
Дальше — каркас Django по `ПЛАН-ПРОЕКТА.md`.
...
Проверка подключения (после `manage.py` в итерации 1):
...
Лимит строки: **119** символов (Ruff / стандарты базы знаний).
...
Папка `_Универсальная-база-знаний/` в корне — локальная база знаний, в Git не попадает (см. `.gitignore`).
